...
public class AlbertoConditionalCostAlgorithm
   ...
   public boolean processAlgorithm() throws GeoAlgorithmExecutionException {
      ...
      for (final IRecordsetIterator iter = movSurfGroupsTable.iterator(); iter.hasNext();) {
         ...
         final IRasterLayer conditional_cost_grid = input_Cond_Costs.get(cost_grid);
         final boolean link = isTrue(values[5].toString());
         int depends_on;
         try {
            depends_on = Integer.parseInt(values[6].toString());
         }
         catch (final Exception e) {
            depends_on = -1;
         }
      ...
      // SET if the surface is associated for other ZDC
      for (final IRecordsetIterator iter = movSurfGroupsTable.iterator(); iter.hasNext();) {
         final IRecord record = iter.next();
         final Object[] values = record.getValues();
         ...
         if (depends_on != -1) {
            final MovementSurface surface = surfacesMap.get(depends_on);
            //surface.setIsAssocietedWithOther(true);
         }
   ...
   private void calculateCost3X3() {
      ...
      while ((m_CentralPoints.size() != 0) && !m_Task.isCanceled()) {
         ...
         while (iter.hasNext()) {
            final Object[] ccsID_GridCell_value = (Object[]) iter.next();
            final String ccsID = (String) ccsID_GridCell_value[0];
            cell = (GridCell) ccsID_GridCell_value[1];
            final Double value = (Double) ccsID_GridCell_value[2];
            x1 = cell.getX();
            y1 = cell.getY();
         ...
         final Object[] ccsID_GridCell_value = (Object[]) m_CentralPoints.get(0);
         m_CentralPoints.remove(0);
         String ccsID = (String) ccsID_GridCell_value[0];
         cell = (GridCell) ccsID_GridCell_value[1];
         final Double value = (Double) ccsID_GridCell_value[2];
         x1 = cell.getX();
         y1 = cell.getY();
         ...
         for (i = -1; i < 2; i++) {
            for (j = -1; j < 2; j++) {
               ...
               final boolean dstDependsOnSomeGZDC = false;
               boolean dstDependsOnOrg = false;
               boolean orgDependsOnDst = false;
               boolean orgDependsOnSomeGZDC = false;
               ...
               for (int k = 0; k < surfacesMap.size(); k++) {
                  ...
                  if (surface.dependsOn(orgSurfaceID)) {
                     someZDdependsOn_ORG = true;
                  }

                  if (surface.dependsOn(dstSurfaceID)) {
                     someZDdependsOn_DST = true;
                  }
                  ...
                  // TODO Quizas se necesite una nueva iteración para las HasSome... y hacer cositas aqui...
               }

               //canMove = canMoveTo(orgSurface, surface.getID());
               ...
               // TODO Es necesario nombrar "Asociado" para que sea unívoco la pertenecia/direccion de la relacion
               // TODO QUIZAS ES el contrario dstSurface.associatedWith(orgSurfaceID);????????????????????????
               if (orgSurface != null) {
                  orgDependsOnSomeGZDC = orgSurface.isDependent();
                  orgDependsOnDst = orgSurface.dependsOn(dstSurfaceID);
                  // TODO QUIZAS ES el contrario ????????????????????????
                  /////////////////////orgDependsOnGZDC = dstSurface.associatedWith(orgSurfaceID);
                  //dstDependsOnSomeGZDC = dstSurface.isAssociatedWithOther();
               }
               if (dstSurface != null) {
                  dstDependsOnOrg = dstSurface.dependsOn(orgSurfaceID);
               }
   ...
   private void setOutputValue(final IRasterLayer outputLayer,
      ...
      if (outputLayer.isNoDataValue(prevDstAccCost) || (prevDstAccCost > dstAccCost)) {
         ...
         m_ClosestPoint.setCellValue(x, y, iPoint);
         final Object[] cssID_GridCell_value = new Object[3];
         cssID_GridCell_value[0] = ccsID;
         cssID_GridCell_value[1] = new GridCell(x, y, iPoint);
         cssID_GridCell_value[2] = dstAccCost;
         ...
         if (!m_AdjPoints_Set.contains(x_y_id)) {
            ...
            m_CentralPoints.add(cssID_GridCell_value);
